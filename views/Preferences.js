    ...
    return (
      ...
      >
        ...
        <View style={styles.fieldWrapper}>
          <Text style={styles.preferenceH}>Pick a genre</Text>
          <View style={[styles.nameInput, { padding: 0 }]}>
            
            <Picker
              selectedValue={genre.genre}
              onValueChange={(itemValue, itemIndex) =>
                setGenre({ genre: itemValue })
              }
            >
              {genres.map((genre, i) => {
                return(
        ...
        <View style={styles.fieldWrapper}>
          <Text style={styles.preferenceH}>Pick a minimum star rating</Text>
          <View style={[styles.nameInput, { padding: 0 }]}>
            <Picker
        ...
        <View style={styles.fieldWrapper}>
          <Text style={styles.preferenceH}>Or search for a keyword</Text>
          <TextInput
...
function loadPreferences(genre, stars, keywords, user) {
  ...
  if (keywords != "Keywords") {
    ...
    if (genre.genre != null && stars.stars == undefined) {
      ...
      query = `/api/movies/filter/language=en-US&page=1&with_genres=${
        genre.genre
      }&vote_average.gte=${stars.stars * 2}`;
    }
  }
  console.log(query);
  user.query = query;
